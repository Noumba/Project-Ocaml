...
let styles = [Passing_style "Pass by value"; Passing_style "Pass by reference"; Passing_style "Pass by Name"; Passing_style "Pass by Copy-Restore"; Passing_style "Pass by Need" ]  (*list of known passing styles*)
...
{fac1 = x ; fac2 = y; fac3=z; fac4=q} -> let paramstyle2 x y z q =   
  ...
  |(Entity_type _,Context_type _,Evaluation_strat _,Correct_type _) -> let new_style user_styles = 
																			let id = Printf.sprintf "%d%d%d%d" a b c d
																			in 
																			match user_styles with 
																			|[] ->let first_style = function 
																				print_string "Enter style name"
																				let name = read_string()
																			in new_style user_styles
