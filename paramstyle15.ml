...
type interpret_rec1 = {styles : style_factors list ; interp : (passing_style * string) list; 
new_entity_instance_list:(string*int) list;new_context_instance_list:(string*int) list;
new_eval_instance_list:(string*int) list;new_typ_instance_list:(string*int) list
; effect : (passing_style * string) list }
...
type new_factor_instance4 = {fact_name:string ; num:int ; new_typ_instance_list:(string*int) list}
...
typ: factors_instance_list; new_entity_instance_list:(string*int) list; new_context_instance_list:(string*int) list;
 new_eval_instance_list:(string*int) list; new_typ_instance_list:(string*int) list }    (**Record type for holding instances of parameter passing style factors*)
...
let interpretation_list = [(Passing_style "PBV", "Evaluation from left to right")]

let effect_list = [(Passing_style "PBV", "No mutation of variables")]
...
(**
Inserts a new passing style in the initial list of of passing style(known passing styles)
...
let get_name entity_type context_type evaluation_strat typing  =   
  ...
  |(Entity_type [1],Context_type [2],Evaluation_strat [3],Correct_type [4]) -> "PBNE"																				
	|(Entity_type _,Context_type _,Evaluation_strat _,Correct_type _) -> let () = print_string "-->Enter name the style: " 
																																								in read_line()
...
let user_facing_information styles interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect= 
...
in 
{styles = styles; interp = interp; new_entity_instance_list=new_entity_instance_list;
new_context_instance_list=new_context_instance_list; new_eval_instance_list = new_eval_instance_list;
new_typ_instance_list = new_typ_instance_list; effect= effect}
...
in if t = 1 then 
 let () = print_string "-->Initialise selected factor(see help menu for possible value): " 
 in let y1 = read_line()
...
let insert_new_interpretation name interpretation_list = 
	let () = print_string "--> Give interpretation: "
...
*)
	let insert_new_passing_style styles interpretation_list effect_list= 
  ...
  in let new_typ_instance_list = record.new_typ_instance_list
	in 
	{ styles = styles ; interp = interp; new_entity_instance_list = new_entity_instance_list; 
	new_context_instance_list  = new_context_instance_list;	new_eval_instance_list = new_eval_instance_list;
	new_typ_instance_list = new_typ_instance_list; effect = effect }
...
*)
let rec delete_passing_style styles interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect= 
...
type view_interpret_rec = {styles : style_factors list ; interp : (passing_style * string) list ; 
known: unit; new_entity_instance_list:(string*int) list; new_context_instance_list:(string*int) list; 
new_eval_instance_list:(string*int) list; new_typ_instance_list:(string*int) list; 
effect : (passing_style * string) list }
...
let rec print_list new_entity_instance_list li = 
	match li with 
	[] -> ()
	|hd :: tl ->  print_string (ful_name(return_factor_instance hd new_entity_instance_list)) ; 
...
let print_list_li_context new_context_instance_list li =
	let rec print_list new_context_instance_list li = 
		match li with 
		[] -> ()
		|hd :: tl ->  print_string (ful_name_context(return_factor_context_instance hd new_context_instance_list)) ;
...
let print_list_li_eval new_eval_instance_list li =
	let rec print_list new_eval_instance_list li = 
		match li with 
		[] -> ()
		|hd :: tl ->  print_string (ful_name_eval(return_factor_eval_instance hd new_eval_instance_list)) ; print_string " "; print_list new_eval_instance_list tl 
		in 
		print_string "[";
...
let print_list_li_typ new_typ_instance_list li =
  let rec print_list new_typ_instance_list li = 
    match li with 
    [] -> ()
    |hd :: tl ->  print_string (ful_name_typ(return_factor_typ_instance hd new_typ_instance_list)) ; print_string " "; print_list new_typ_instance_list tl 
    in 
    print_string "[";
	...
	|{name = Passing_style a;	factor ={entity = Entity_type b; context = Context_type c; evaluation = Evaluation_strat d;
		typing = Correct_type e}} :: tl
		 -> print_endline "{name = "; print_string (ful_name_style a); print_endline ";"; 
										...
										print_string "; evaluation = ";print_list_li_eval new_eval_instance_list d;print_string " typing = "; print_list_li_typ new_typ_instance_list e;print_endline "}};";
										print_style_list tl
						in print_string "[";
...
let view_specific_passing_style style interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect_list=
	let known = 
	view_style style interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect_list
	in let styles = style 
	in let interp = interpretation_list
	in let new_entity_instance_list = new_entity_instance_list
	in {styles = styles;interp=interp;known= known; new_entity_instance_list = new_entity_instance_list; 
	new_context_instance_list = new_context_instance_list;new_eval_instance_list = new_eval_instance_list;
	new_typ_instance_list; effect = effect_list}

let display_styles styles interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect= 
let styles =  styles
in let () = print_styles styles new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list
in let interp = interpretation_list 
...
new_eval_instance_list = new_eval_instance_list; new_typ_instance_list = new_typ_instance_list ; effect = effect}
;;
...
*)
let rec usermind user_styles interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect_list= 
let () = display_menu () 
in let () = print_string " Enter your choice (1-6): "
in let opinion  = validate_input() in 
  match opinion with
  |1 -> let update = (insert_new_passing_style user_styles interpretation_list effect_list ) in
  
          usermind update.styles update.interp update.new_entity_instance_list update.new_context_instance_list update.new_eval_instance_list update.new_typ_instance_list update.effect
          
  |2 ->let update = (delete_passing_style user_styles interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect_list) in	
                
        usermind  update.styles update.interp update.new_entity_instance_list update.new_context_instance_list update.new_eval_instance_list update.new_typ_instance_list update.effect
        
  |3 ->let update = (display_styles user_styles interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect_list) in 
  
        usermind update.styles update.interp update.new_entity_instance_list update.new_context_instance_list update.new_eval_instance_list update.new_typ_instance_list update.effect
        
  |4 ->let update = (view_specific_passing_style user_styles interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect_list)	in
    
        usermind update.styles update.interp update.new_entity_instance_list	update.new_context_instance_list update.new_eval_instance_list update.new_typ_instance_list update.effect
        
  |5 ->let update = (user_facing_information user_styles interpretation_list new_entity_instance_list new_context_instance_list new_eval_instance_list new_typ_instance_list effect_list) in
...
usermind user_styles interpretation_list record.new_entity_instance_list record.new_context_instance_list record.new_eval_instance_list record.new_typ_instance_list effect_list
;;
